...
optuna_logger.change_dirpath(cwd + "/" + "CTGAN_" + optuna_logger.PID)
...
def objective(trial):
    ...
    # initialize a new ctga  model
    global ctgan_mdl
    ctgan_mdl = CTGANSynthesizer()
    ...
    ctgan_mdl.logger.write_to_file('Both seed number ' + str(this_seed))
    ...
    return ctgan_mdl.prop_dis_validation
...
def callback(study, trial):
    ...
    if trial.state == optuna.trial.TrialState.COMPLETE:
        this_model_fn = "CTGAN_model_" + str(trial.number) + "_" \
                        + optuna_logger.PID + "_" \
                        + optuna_logger.dt.now().strftime(optuna_logger.datetimeformat)

        if len(metric_vals) < num_max_mdls:
            metric_vals.append(ctgan_mdl.prop_dis_validation)
            mdl_fns.append(this_model_fn)
            metric_vals, mdl_fns = sortlists(metric_vals, mdl_fns)

            ctgan_mdl.save(optuna_logger.dirpath + "/" + this_model_fn + ".pkl")
        else:
            print(mdl_fns)
            if ctgan_mdl.prop_dis_validation < metric_vals[-1]:
