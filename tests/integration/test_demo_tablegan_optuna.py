...
optuna_logger.change_dirpath(cwd + "/" + "TableGAN" + optuna_logger.PID)
...
def objective(trial):
    ...
    # initialize a new tablegan model
    global tablegan_mdl
    tablegan_mdl = TableganSynthesizer()

    # Create a new folder to save the training results
    tablegan_mdl.logger.change_dirpath(tablegan_mdl.logger.dirpath + "/TableGAN" + tablegan_mdl.logger.PID)  ## create a folder with PID

    # Record the seed number
    tablegan_mdl.logger.write_to_file('Both seed number ' + str(this_seed))

    # NOTE: to use Optuna, pass trial to fit function
    tablegan_mdl.fit(data, discrete_columns, model_summary=False, trans="VGM", trial=trial)

    return tablegan_mdl.prop_dis_validation
...
def callback(study, trial):
    ...
    if trial.state == optuna.trial.TrialState.COMPLETE:
        this_model_fn = "TableGAN_model_" + str(trial.number) + "_" \
                        + optuna_logger.PID + "_" \
                        + optuna_logger.dt.now().strftime(optuna_logger.datetimeformat)

        if len(metric_vals) < num_max_mdls:
            metric_vals.append(tablegan_mdl.prop_dis_validation)
            mdl_fns.append(this_model_fn)
            metric_vals, mdl_fns = sortlists(metric_vals, mdl_fns)

            tablegan_mdl.save(optuna_logger.dirpath + "/" + this_model_fn + ".pkl")
        else:
            print(mdl_fns)
            if tablegan_mdl.prop_dis_validation < metric_vals[-1]:
...
if __name__ == "__main__":
    ...
    # mdl_fn = "TableGAN_model" + best_mdl.logger.PID + "_" + best_mdl.logger.datetimeval + ".pkl"
